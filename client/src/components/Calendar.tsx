...
export function Calendar({ month, onMonthChange, shifts, armedTemplateId, templates, onBatch, onDeleteShift, onUpdateShift, onSelectShift }: Props) {
  ...
  const handleMouseUp = useCallback(async (dateStr: string) => {
    ...
    if (start === end) {
      ...
      if (template) {
        const mode = existingShift?.templateId === template.id ? "REMOVE" : "SET";
        await onBatch({ mode, templateId: template.id, dates: [end] });
      } else {
        setDragPreview(null);
        setSelDates(new Set());
        if (existingShift) {
